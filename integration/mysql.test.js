    ...
    test(t.query, async () => {
      await withDocker({
        ...
        wait: async (cid) => {
          while (true) {
            try {
              cp.execSync('docker exec ' + cid + ' mysql -h localhost -u root -proot --execute="SELECT 1"');
              return;
            } catch (e) {
              if (String(e).includes(`ERROR 2002 (HY000): Can't connect to local MySQL server through socket '/var/run/mysqld/mysqld.sock' (2)`) ||
                  String(e).includes(`ERROR 1045 (28000): Access denied for user 'root'@'localhost' (using password: YES)`)) {
